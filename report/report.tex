...
\end{lstlisting}
Higher-kinded types allow the programmer to abstract over type constructors. For example, we can write a utility function that converts to elements implementing the \texttt{Show} typeclass to strings in any (data) structure implementing the \texttt{Functor} typeclass:
\begin{lstlisting}
...
JavaScript has also no kind system, it does not even have a static type system. To achieve kind-like reasoning, we can employ the same methods previously described for F\# in TypeScript. For the first variant using a generic type with two type variables, one being a "witness/tag" type and the other type variable being the element type, we can model it in TypeScript:
\begin{lstlisting}
